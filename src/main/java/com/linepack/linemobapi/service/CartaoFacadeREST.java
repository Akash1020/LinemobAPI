...
public class CartaoFacadeREST extends AbstractFacade<Cartao> {
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") BigDecimal id, Cartao entity) {
        super.edit(entity);
    }
    ...
    public Cartao find(@PathParam("id") BigDecimal id) {
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Cartao> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
